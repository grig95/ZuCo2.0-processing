...
def get_bad_channels(idx, subject, task = "sentiment"):
    ...
    bad_channels = bad_channels[()]s[0].split(" ") if bad_channels[()]s else None
    return bad_channels
"""
...
def extract_word_order_from_fixations(fixations_order_per_word):
    ...
    for fixation_n in range(n_tot_fixations):
        mins_per_word_idx = np.array([np.min(i) if len(i)>0 else np.nan for i in fxs_list])
        next_word_fixated = int(np.nanargmin(mins_per_word_idx)) # Seems to work like this
        fxs_list[next_word_fixated].remove(np.min(fxs_list[next_word_fixated]))
...
def extract_word_level_data(data_container, word_objects, eeg_float_resolution = np.float16):
    ...
    if "rawEEG" in available_objects:
        ...
        for raw_eegs_obj, ets_obj, word_obj, ffd, gd, gpt, trt, nFix, fixPos, trt_t1, trt_t2, trt_a1, trt_a2, trt_b1, trt_b2, trt_g1, trt_g2 in zipped_data:
            ...
            #print(data_container[trt_t1[0]][()].shape)
            data_dict["TRT_t1"] = data_container[trt_t1[0]][()] if len(data_container[trt_t1[0]][()].shape) == 2 else None
            data_dict["TRT_t2"] = data_container[trt_t2[0]][()] if len(data_container[trt_t2[0]][()].shape) == 2 else None
            data_dict["TRT_a1"] = data_container[trt_a1[0]][()] if len(data_container[trt_a1[0]][()].shape) == 2 else None
            data_dict["TRT_a2"] = data_container[trt_a2[0]][()] if len(data_container[trt_t2[0]][()].shape) == 2 else None
            data_dict["TRT_b1"] = data_container[trt_b1[0]][()] if len(data_container[trt_b1[0]][()].shape) == 2 else None
            data_dict["TRT_b2"] = data_container[trt_b2[0]][()] if len(data_container[trt_b2[0]][()].shape) == 2 else None
            data_dict["TRT_g1"] = data_container[trt_g1[0]][()] if len(data_container[trt_g1[0]][()].shape) == 2 else None
            data_dict["TRT_g2"] = data_container[trt_g2[0]][()] if len(data_container[trt_g2[0]][()].shape) == 2 else None
    ...
    for word_idx in word_reading_order:
        word_level_data[word_idx]['reading_order'] = word_idx
